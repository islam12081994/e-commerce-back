...
const orderAddressFor = (user: any) => ({
...
const seed = async (): Promise<void> => {
  ...
  appLogger.info('Connected to MongoDB for seeding');

  const shouldWipe = process.argv.includes('--wipe');
  if (shouldWipe) {
    appLogger.info('Wiping existing data...');
    await Promise.all([
      User.deleteMany({}),
      Product.deleteMany({}),
      Category.deleteMany({}),
  ...
  if (existingUsers > 0 && !shouldWipe) {
    appLogger.info(`Database already has ${existingUsers} users. Use --wipe to reset.`);
    await mongoose.disconnect();
    return;
  }

  appLogger.info('Creating categories...');
  const categories: any = {};
  for (const cat of categoriesData) {
    const created = await Category.create(cat);
    categories[cat.name] = created;
    appLogger.info(`  + Category ${cat.name}`);
  }

  appLogger.info('Creating products...');
  const products: any = {};
  for (const prod of productsData) {
    ...
    appLogger.info(`  + Product ${prod.name}`);
  }
  ...
  for (const u of usersData) {
    ...
    appLogger.info(`  + User ${u.email} (${u.role})`);
  }

  appLogger.info('Creating demo orders...');
  const orderSeeds = [
    {
  ...
  for (const seedOrder of orderSeeds) {
    const orderItems = seedOrder.items.map((item: any) => ({
    ...
    appLogger.info(`  + Order ${order._id} for ${seedOrder.user.email} (${seedOrder.status})`);
  }

  appLogger.info('Adding initial security events...');
  const bootEvents = [
    { eventType: SecurityEventType.LOGIN_SUCCESS, user: users['admin@example.com'], sev: 'LOW' as const, details: 'initial admin login seed' },
    { eventType: SecurityEventType.LOGIN_SUCCESS, user: users['alice@example.com'], sev: 'LOW' as const, details: 'initial alice login seed' },
    { eventType: SecurityEventType.LOGIN_SUCCESS, user: users['bob@example.com'], sev: 'LOW' as const, details: 'initial bob login seed' },
    { eventType: SecurityEventType.FORBIDDEN_ACCESS, user: undefined, sev: 'HIGH' as const, details: 'simulated unauthorized admin attempt seed' },
    { eventType: SecurityEventType.RATE_LIMIT_EXCEEDED, user: undefined, sev: 'HIGH' as const, details: 'simulated brute force attempt seed' },
  ];
